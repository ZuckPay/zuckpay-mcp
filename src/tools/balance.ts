 ...
 * (total, disponível pra saque e bloqueado D+2) e os limites de saque
 * vigentes pra essa conta. Nenhum dado de terceiros trafega aqui.
 */
...
async function handleGetBalance(client: ZuckPayClient, args: unknown): Promise<ToolResult> {
  return safeRun(client.redact, async () => {
    ...
    if (locked !== undefined) {
      lines.push(`• Bloqueado (liberação D+2): ${formatMoney(locked, currency)}`);
    }
    if (total !== undefined) {
...
export function registerBalanceTool(server: McpServer, client: ZuckPayClient): void {
  server.registerTool(
    "getBalance",
    {
      ...
      description:
        "Consulta os saldos da conta ZuckPay autenticada: disponível para saque, bloqueado (liberação D+2) e total, " +
        "além dos limites e taxa de saque vigentes. Somente leitura, sem parâmetros.",
      inputSchema: {},
      annotations: {
        readOnlyHint: true,
        openWorldHint: true,
